...
#[derive(Debug)]
pub(in crate::s7_protocol) struct ReadWriteParams<'a> {
    ...
    pub(in crate::s7_protocol) request_item: Option<&'a [RequestItem]>, // This structure is used to address the actual variables,
...
impl ReadWriteParams<'_> {
    pub(in crate::s7_protocol) fn len() -> usize {
        2
    }
}

impl From<&mut BytesMut> for ReadWriteParams<'_> {
...
impl<'a> From<ReadWriteParams<'a>> for BytesMut {
    fn from(req_item: ReadWriteParams<'a>) -> BytesMut {
